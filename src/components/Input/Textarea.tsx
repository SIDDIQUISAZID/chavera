...
const Textarea = forwardRef(
    ({ name, type, errors, children, wrapperClass = "", isLoading, className, prefix, wrapperAttr, ...props }: InputProps, ref: React.ForwardedRef<HTMLTextAreaElement>) => {
        ...
        return (
            <div {...wrapperAttr} className={cn("mb-3 w-full", wrapperClass)} >
                <div className="relative h-full">
                    ...
                    {children && <label
                        htmlFor={name}
                        className="absolute  rounded-2xl   font-medium  text-theme-black duration-300 transform -translate-y-4 scale-75 -top-2 z-10 origin-[0]  px-2 peer-focus:px-2 peer-focus:text-blue-dark text-blue-dark peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 -left-1"
                    >
                        {children}
                        {props?.required && <span className="text-red-600">*</span>}
                        {/* Your E-mail <span className="text-danger">*</span> */}
                    </label>}
                    <span className="absolute -bottom-4 left-4  text-red-600 text-[9px] xs:text-xs">{errors?.[name]?.message}</span>
                </div>
            </div>
        );
    }
